...
if (empty($rzp_order_id)) {
    $rzp_order = razorpay_create_order($order['total'], $order['order_code'] ?: ('BO-' . $order_id), [
        'order_id' => $order_id,
        'customer_id' => $order['customer_id']
    ]);
    if (!empty($rzp_order['order_id'])) {
        $rzp_order_id = $rzp_order['order_id'];
        $con->query("UPDATE orders SET razorpay_order_id = '$rzp_order_id' WHERE id = $order_id");
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Pay Order #<?php echo htmlspecialchars($order['order_code'] ?: 'BO-'.$order['id']); ?> | Biteora</title>
...
<body style="background:#FFF8F3; min-height:100vh; padding:1.5rem 1rem; display:flex; flex-direction:column; align-items:center; justify-content:center;">

  <div class="card" style="width:100%; max-width:440px; padding:2rem 1.5rem; background:#FFFFFF; border:1px solid var(--border-subtle); border-radius:24px; box-shadow:var(--shadow-lg); text-align:center;">
    
    <!-- Brand Header -->
    <div style="display:flex; align-items:center; justify-content:center; gap:0.6rem; margin-bottom:1.25rem;">
      <div class="cb-logo-icon" style="width:38px; height:38px; font-size:1.15rem; background:linear-gradient(135deg, #FF5A36, #FF7048);">
        <i class="fa-solid fa-utensils"></i>
      </div>
      <span style="font-family:var(--font-display); font-size:1.4rem; font-weight:800; color:var(--text-primary);">Bite<span class="text-orange">ora</span></span>
    </div>
    ...
    <h2 style="font-family:var(--font-display); font-size:1.6rem; font-weight:800; color:var(--text-primary); margin-bottom:1.25rem;">
      Order #<?php echo htmlspecialchars($order['order_code'] ?: 'BO-'.$order['id']); ?>
    </h2>

    <!-- Price Card -->
    <div style="background:var(--bg-surface-2); border:1px solid var(--border-subtle); border-radius:16px; padding:1.25rem; margin-bottom:1.25rem;">
      <div style="font-size:0.8rem; color:var(--text-secondary); text-transform:uppercase; font-weight:700; margin-bottom:0.25rem;">
        Total Amount Payable
      </div>
      <div style="font-family:var(--font-display); font-size:2.4rem; font-weight:900; color:var(--brand-primary); letter-spacing:-0.5px;">
      ...
      <div style="display:flex; align-items:center; justify-content:center; gap:0.5rem; margin-top:0.4rem; font-size:0.8rem; color:var(--text-secondary);">
        ...
        <span id="mobile-payment-badge"><?php echo biteora_render_payment_badge($order['payment_status'] ?? 'Pending'); ?></span>
      </div>
    </div>

    <!-- Order Items Accordion/Preview -->
    <div style="background:var(--bg-surface-2); border:1px solid var(--border-subtle); border-radius:14px; padding:1rem; margin-bottom:1.5rem; text-align:left;">
      ...
      <div style="display:flex; flex-direction:column; gap:0.5rem;">
        <?php foreach ($items as $item): ?>
          <div style="display:flex; justify-content:space-between; font-size:0.9rem; color:var(--text-primary);">
            ...
            <span style="font-weight:700; color:var(--brand-primary);">₹<?php echo number_format($item['price'], 2); ?></span>
          </div>
        <?php endforeach; ?>
      </div>
    </div>
    ...
    <button type="button" id="pay-now-btn" onclick="startMobilePayment();" class="btn btn-primary btn-lg" style="width:100%; font-size:1.15rem; font-weight:800; padding:1rem; border-radius:14px; box-shadow:var(--shadow-orange); margin-bottom:1rem;">
  ...
  function startMobilePayment() {
    ...
    const rzpOptions = {
      ...
      currency: "INR",
      name: "Biteora",
      description: "Order #<?php echo htmlspecialchars($order['order_code'] ?: 'BO-'.$order['id']); ?>",
      order_id: "<?php echo $rzp_order_id; ?>",
      prefill: {
        name: "<?php echo addslashes($order['customer_name'] ?? 'Student'); ?>",
        email: "<?php echo addslashes($order['customer_email'] ?? 'student@biteora.edu'); ?>",
      ...
      handler: function (response) {
        ...
        .then(data => {
          if (data.success) {
            ...
            errBox.style.display = 'block';
            btn.removeAttribute('disabled');
            btn.innerHTML = '<i class="fa-solid fa-rotate-right"></i> Retry Payment (₹<?php echo $amount_formatted; ?>)';
          }
        })
        .catch(e => {
      ...
      modal: {
        ondismiss: function () {
          // Record cancellation/dismissal
          const cfd = new FormData();
          cfd.append('order_id', "<?php echo $order_id; ?>");
          cfd.append('event', 'cancel');
          fetch('api/record-payment-event.php', { method: 'POST', body: cfd }).catch(() => {});

          btn.removeAttribute('disabled');
          btn.innerHTML = '<i class="fa-solid fa-rotate-right"></i> Retry Payment (₹<?php echo $amount_formatted; ?>)';
          errBox.innerHTML = '⚠️ Payment was cancelled or closed. Your order remains pending. You can retry paying anytime.';
          errBox.style.display = 'block';
          errBox.style.background = '#FFFBEB';
          errBox.style.borderColor = '#FDE68A';
          errBox.style.color = '#B45309';

          const badgeEl = document.getElementById('mobile-payment-badge');
          if (badgeEl) badgeEl.innerHTML = '<span class="badge badge-secondary" style="background:#F1F5F9; color:#64748B; border:1px solid #CBD5E1; font-weight:700; display:inline-flex; align-items:center; gap:5px;"><i class="fa-solid fa-ban"></i> Cancelled</span>';
        }
      }
    };
    ...
    try {
      ...
      rzp.on('payment.failed', function (resp) {
        ...
        ffd.append('order_id', "<?php echo $order_id; ?>");
        ffd.append('event', 'fail');
        ffd.append('reason', errDesc);
        fetch('api/record-payment-event.php', { method: 'POST', body: ffd }).catch(() => {});

        btn.removeAttribute('disabled');
        btn.innerHTML = '<i class="fa-solid fa-rotate-right"></i> Retry Payment (₹<?php echo $amount_formatted; ?>)';
        errBox.innerHTML = '❌ ' + errDesc + '. Please retry payment.';
        errBox.style.display = 'block';
        ...
        const badgeEl = document.getElementById('mobile-payment-badge');
        if (badgeEl) badgeEl.innerHTML = '<span class="badge badge-danger" style="background:#FEF2F2; color:#DC2626; border:1px solid #FECACA; font-weight:700; display:inline-flex; align-items:center; gap:5px;"><i class="fa-solid fa-circle-xmark"></i> Failed</span>';
      });
      rzp.open();
    } catch (e) {
      btn.removeAttribute('disabled');
