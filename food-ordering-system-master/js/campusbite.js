/**
 * CAMPUSBITE - Food-Tech Client Controller & Live Poller
 */
...
const CampusBite = {
      ...
      html += `
        <div class="cart-item-row" style="display:flex; align-items:center; justify-content:space-between; padding:0.85rem 0; border-bottom:1px solid var(--border-subtle);">
          <div style="display:flex; align-items:center; gap:0.85rem;">
            ...
            <div>
              <div style="font-weight:700; font-size:0.92rem; color:#FFFFFF;">${item.name}</div>
          ...
          <div style="display:flex; align-items:center; gap:1.15rem;">
            <div class="qty-control">
              <button type="button" class="qty-btn" onclick="CampusBite.updateQty(${item.id}, -1)">-</button>
              <span class="qty-val">${item.qty}</span>
              <button type="button" class="qty-btn" onclick="CampusBite.updateQty(${item.id}, 1)">+</button>
            </div>
            <div style="font-weight:800; font-family:var(--font-display); min-width:65px; text-align:right; color:#FFFFFF;">₹${itemTotal}</div>
            <button type="button" onclick="CampusBite.removeItem(${item.id})" style="background:none; border:none; color:var(--danger); cursor:pointer; padding:0.3rem;" title="Remove">✕</button>
          </div>
        </div>
      `;
    });
  ...
  showToast(msg, type = 'success') {
    ...
    toast.style.cssText = 'padding:0.75rem 1.15rem; font-size:0.85rem; border-radius:10px; background:var(--bg-surface); border:1px solid var(--border-subtle); color:#FFFFFF; box-shadow:var(--shadow-lg); animation:fadeIn 0.2s ease; margin:0;';
  ...
  askAI(query) {
    ...
    resultsContainer.innerHTML = '<div style="text-align:center; padding:1.5rem; color:var(--text-secondary);">🤖 CampusBite AI is searching live cafeteria matches...</div>';
          ...
          html += `
            <div style="display:flex; align-items:center; justify-content:space-between; background:var(--bg-surface-2); border:1px solid var(--border-subtle); padding:0.65rem 0.85rem; border-radius:10px;">
              <div style="display:flex; align-items:center; gap:0.65rem;">
                ...
                <div>
                  <div style="font-weight:700; font-size:0.88rem; color:#FFFFFF;">${item.name}</div>
                  <div style="font-size:0.75rem; color:var(--text-secondary);">${item.category_name || 'Cafeteria'} • ⭐ ${item.rating || '4.8'}</div>
                </div>
              </div>
              <div style="display:flex; align-items:center; gap:0.65rem;">
                <span style="font-weight:800; font-family:var(--font-display); color:#FFFFFF;">₹${parseFloat(item.price).toFixed(2)}</span>
                <button type="button" onclick="CampusBite.addToCart(${item.id}, '${item.name.replace(/'/g, "\\'")}', ${item.price}, '${item.image}', ${item.is_veg});" class="btn btn-primary btn-sm" style="font-size:0.75rem; padding:0.35rem 0.65rem;">Add +</button>
              </div>
            </div>
          `;
        });
