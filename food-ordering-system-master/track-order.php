...
<div class="container" style="padding: 2.5rem 1.5rem 5rem;">
  ...
  <?php if ($is_delivery): ?>
    ...
    <div id="cb-delivery-out-alert" style="<?php echo ($current_status === 'Out for Delivery') ? '' : 'display:none;'; ?> padding:1.25rem 1.5rem; margin-bottom:1.75rem; border-radius:16px; border:1.5px solid var(--brand-primary); background:#FFF0EA; display:flex; align-items:center; gap:1rem;">
      ...
      <div>
        <h3 style="font-family:var(--font-display); font-weight:900; font-size:1.25rem; color:var(--text-primary); margin-bottom:0.15rem;">🚚 OUT FOR DELIVERY!</h3>
        <p style="font-size:0.9rem; color:var(--text-secondary);">Your Biteora order is on the way to <strong><?php echo htmlspecialchars($order['delivery_room'] ?: 'your classroom'); ?></strong>.</p>
      </div>
    </div>
